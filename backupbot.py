...
def save_friends():
    ...
    for friend in friends.json():
        if friend['type'] == 1:
            ...
            with open('Backup/Friends.txt', 'a', encoding = 'UTF-8') as f:
                f.write(username)
            saved_friends += 1

    with open('Backup/Friends.txt', 'r', encoding = 'UTF-8') as f:
        fixed = f.read()[:-1]
    with open('Backup/Friends.txt', 'w', encoding = 'UTF-8') as f:
        f.write(fixed)
...
def save_servers():
    ...
    servers = requests.get('https://discordapp.com/api/v8/users/@me/guilds', headers = headers)
    ...
    for server_info in server_info_all.splitlines():
        ...
        channels = requests.get('https://discord.com/api/v8/guilds/%s/channels' % (server_id), headers = headers)
        for channel in channels.json():
            if channel['type'] == 0:
                channel_id = channel['id']
                invite = requests.post('https://discord.com/api/v8/channels/%s/invites' % (channel_id), json = payload, headers = headers)
                ...
                if invite.status_code == 403:
                    ...
                    if attempts == 5:
                        ...
                        with open('Backup/Discord Servers.txt', 'a', encoding = 'UTF-8') as f:
                            f.write('Backup/Discord Server: %s | Vanity URL\n' % (server_name))
                        saved_servers += 1
                        attempts = 0
                        break
                    else:
                    ...
                    sys.stdout.write('Backup/Discord Server: %s | Invite Link: %s\n' % (server_name, invite_url))
                    with open('Backup/Discord Servers.txt', 'a', encoding = 'UTF-8') as f:
                        f.write('Backup/Discord Server: %s | Channel ID: %s | Invite Link: %s\n' % (server_name, channel_id, invite_url))
                    saved_servers += 1
                    time.sleep(4)
                    break
        ...
        with open('Discord Friends.txt', 'r', encoding = 'UTF-8') as f:
            for line in f.readlines():
                while True:
                    ...
                    add = requests.put('https://discord.com/api/v8/users/@me/relationships/%s' % (user_id), json = {}, headers = headers)
        ...
        with open('Discord Servers.txt', 'r', encoding = 'UTF-8') as f:
            for line in f.readlines():
                while True:
                    ...
                    join = requests.post('https://discord.com/api/v8/invites/%s' % (invite_code), headers = headers)
...
while True:
    ...
    connect = requests.get('https://canary.discordapp.com/api/v8/users/@me', headers = headers)
